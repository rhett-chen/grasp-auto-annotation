...
struct HashTable {
  ...
  void init(unsigned int expected_size) {
    ...
    free_list = 0;
    for (unsigned int i = 0; i < pool.size() - 1; ++i) pool[i].next = i + 1;
  ...
  void reserve(unsigned int expected_size) {
    ...
    table.resize(1u << table_rank);
    unsigned int t;
    for (t = old_size; t < table.size(); ++t) table[t] = -1;  // initially make new lists empty
    int j, *p_j;
  ...
  Data &operator()(const Key &k, const Data &missing_data) {
    ...
    add(k,
        missing_data);  // note - this could cause the table to be resized, and t made out-of-date
