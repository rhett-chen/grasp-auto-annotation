...
struct HashGrid2 {
  ...
  void add_point(const Vec2d &x, const DataType &datum) { grid.add(round(x * overdx), datum); }
  ...
  void add_box(const Vec2d &xmin, const Vec2d &xmax, const DataType &datum) {
    ...
    for (int j = imin[1]; j <= imax[1]; ++j)
      for (int i = imin[0]; i <= imax[0]; ++i) grid.add(Vec2i(i, j), datum);
  }

  void delete_box(const Vec2d &xmin, const Vec2d &xmax, const DataType &datum) {
    Vec2i imin = round(xmin * overdx), imax = round(xmax * overdx);
    for (int j = imin[1]; j <= imax[1]; ++j)
      for (int i = imin[0]; i <= imax[0]; ++i) grid.delete_entry(Vec2i(i, j), datum);
  }

  unsigned int size(void) const { return grid.size(); }

  void clear(void) { grid.clear(); }

  void reserve(unsigned int expected_size) { grid.reserve(expected_size); }
  ...
  bool find_box(const Vec2d &xmin, const Vec2d &xmax, std::vector<DataType> &data_list) const {
    ...
    Vec2i imin = round(xmin * overdx), imax = round(xmax * overdx);
    for (int j = imin[1]; j <= imax[1]; ++j)
      for (int i = imin[0]; i <= imax[0]; ++i) grid.append_all_entries(Vec2i(i, j), data_list);
    return data_list.size() > 0;
  }
};
...
struct HashGrid3 {
  ...
  void add_point(const Vec3d &x, const DataType &datum) { grid.add(round(x * overdx), datum); }
  ...
  void add_box(const Vec3d &xmin, const Vec3d &xmax, const DataType &datum) {
    ...
    for (int k = imin[2]; k <= imax[2]; ++k)
      for (int j = imin[1]; j <= imax[1]; ++j)
        for (int i = imin[0]; i <= imax[0]; ++i) grid.add(Vec3i(i, j, k), datum);
  }
  ...
  void delete_box(const Vec3d &xmin, const Vec3d &xmax, const DataType &datum) {
    ...
    for (int k = imin[2]; k <= imax[2]; ++k)
      for (int j = imin[1]; j <= imax[1]; ++j)
        for (int i = imin[0]; i <= imax[0]; ++i) grid.delete_entry(Vec3i(i, j, k), datum);
  }
  ...
  bool find_box(const Vec3d &xmin, const Vec3d &xmax, std::vector<DataType> &data_list) const {
    ...
    for (int k = imin[2]; k <= imax[2]; ++k)
      for (int j = imin[1]; j <= imax[1]; ++j)
        for (int i = imin[0]; i <= imax[0]; ++i) grid.append_all_entries(Vec3i(i, j, k), data_list);
    return data_list.size() > 0;
  }
};
